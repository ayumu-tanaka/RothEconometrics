...
\begin{frame}{コースの概要}
	\begin{wideitemizeshort}
		...
		\pause
		\item
		因果的効果の考え方を、\textit{潜在的結果}（potential outcomes）を用いて定式化しました。
			\begin{itemize}
				\item 
...
データで因果的な問いに答えるには、2つの主要な課題があります： \medskip 
	...
	\begin{wideitemizeshort}
		\item
		各ユニットについて、反実仮想的な結果を観察することは決してできません。
			\begin{itemize}
				\item
...
\begin{frame}{差の差分析 I}
	\centering
	\includegraphics[width = 0.55\linewidth]{../Chapter6/hastings-event-study-w-labels}
	...
	\begin{wideitemizeshort}
		...
		\pause
		\item
		識別：並行トレンドの下で、 $\tau_{ATT} = (\mu_{12} - \mu_{11}) - (\mu_{02} - \mu_{01})$
