...
\begin{wideitemize}
	...
	\textbf{事前反応なしの仮定} (No anticipation)： $Y_{i1}(0) = Y_{i1}(1)$
		...
		\end{itemize}	
	\item
	\textbf{平行トレンド仮定} (Parallel trends)：
...
\begin{wideitemize}
	...
	&= E[Y_{i2}(1) - Y_{i2}(0) | D_i = 1] \text{ \scriptsize(平行トレンド) }
	\end{align*}
...
\begin{frame}{ATT の推定}
	\begin{wideitemize}
		\item
		DID の仮定（平行トレンドと事前反応なし）の下で、ATT は次のように識別されることを示しました：
...
\begin{frame}{多期間における DID}
	...
	\begin{wideitemize}
		...
		\begin{enumerate}
			\item 
			処置\textit{前}に平行トレンドが成り立っているように見えるかをテストできます。\pause{}
...
\begin{frame}{平行トレンドに関する注意点}
	
	\begin{wideitemize}
		\item
		DID は平行トレンド仮定に依存しています。これは選択バイアスを許容しますが、それが時間を通じて一定であることを要求します。 \pause これは、時間とともに変化する交絡因子の存在を排除しています。
